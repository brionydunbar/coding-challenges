"""
Have the function SwitchSort(arr) take arr which will be an array consisting of integers 1...size(arr) and determine what the fewest number of steps is in order to sort the array from least to greatest using the following technique:
...
def SwitchSort(arr):
  ...
  # hashing elements with correct positions
  pos = {}
  ...
  for i in range(length):
    if temp[i] != arr[i]:
      # index of element that should be at index i
      idx = pos[temp[i]]
...
print(SwitchSort(arr1))
print(SwitchSort(arr2))